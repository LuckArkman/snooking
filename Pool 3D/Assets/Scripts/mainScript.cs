...
public class mainScript : MonoBehaviour
{
	...
	[Range(0.00f, 10.0f)]
	public float taco_smooth_position = 0.025f;
	[Range(0.0f, 20.0f)]
	public float taco_distance = 1.0f;
	[Range(-0.5f, 0.5f)]
	public float taco_direcx = 0.0f;
	[Range(0.0f, 100.0f)]
	public float taco_height = 3.0f;
	public Vector3 taco_lookVel = Vector3.zero;
	public Vector3 taco_posVel = Vector3.zero;
	[Range(0.00f, 10.0f)]
	public float taco_smooth = 0.5f;
	[Range(0.0f, 20.0f)]
	...
	private void Update()
	{
		if (!aiPlaying && stopBalls)
		{
			if (Input.GetMouseButton(0))
			{
				rotationx += Input.GetAxis("Mouse X") * inputSense * Time.deltaTime;
				rotationy += Input.GetAxis("Mouse Y") * inputSense * Time.deltaTime;
			}
		}
	...
	{
		// MPLopes Functions
		// // Não executa mais nenhuma ação se estiver sobre um objeto de UI
		if (UnityEngine.EventSystems.EventSystem.current.IsPointerOverGameObject()) return;
		/*
		cueParentObjTransform.transform.LookAt(GetTacoLookState());
		Vector3 direction = new Vector3(taco_direcx, taco_height, -taco_distance);
		Vector3 rotation = new Vector3(0, rotationx, 0);
		this.cueParentObjTransform.position = Vector3.SmoothDamp(this.cueParentObjTransform.position, TacoPos(Quaternion.Euler(rotation), direction), ref taco_posVel, taco_smooth_position);
		*/
		// MPLopes Functions
		if (Input.GetMouseButton(0))
		{
			Vector3 mousePosition = new Vector3(rotationx, 0.0f, 0.0f);
	...
	// MPLopes Functions
	private Vector3 TacoPos(Quaternion rotation, Vector3 direction)
		=> this.transform.position + rotation * direction;
	
	private Vector3 GetTacoLookState()
		=> Vector3.SmoothDamp(this.cueParentObjTransform.position, transform.position,ref taco_lookVel, taco_smooth);
