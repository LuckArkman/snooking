...
public class mainScript : GenericMain
{
	private void Start()
	{
		...
		playerNames = new string[2] {playerNames[0], avatarNames[chosenAvatar[1]]};
		...
		consoleInit();
		new FindGameObjectsAndComponents().Find(this);
		new FindUIObjectsAndComponents().Find(this);
		switchRulesGroup(0);
	...
	{
		GetAllBallsVelocity();
		_camController.showTaco = !aiPlaying && stopBalls;
		if (aiPlaying) _camController._camTypes = CamTypes.aiPoint;
		if (!aiPlaying) _camController._camTypes = CamTypes.None;
		GetVelocity();
		ShowAndHideSlide();
		OnFixedUpdate();
		updateMenuSystem();
		...
		if (Input.GetMouseButtonDown(0) && curScreen != "InGame")
		{
			...
			uiTouchParticleRectTrans.anchoredPosition = new Vector2(ray.direction.x * canvasSize.width - iPhoneXSideIndent, vector.y * canvasSize.height);
		...
		if (curScreen == "InGame")
		{
			...
			if (bBallInHand)
			{
				...
				if (!aiPlaying)
				{
					...
					transform.position = new Vector3(ray.direction.x, y, position4.z);
		...
		{
			cameraObjTransform.localPosition += Vector3.SmoothDamp(cameraObjTransform.localPosition, Vector3.zero, ref camLocalPosVel, 0.02f);
			setCameraLocalRotSmooth(0.2f);
		}
		else if (cameraObjTransform.parent == cameraFreeViewParentObjTransform)
		{
			cameraObjTransform.localPosition += Vector3.SmoothDamp(cameraObjTransform.localPosition, new Vector3(0f, 2f, 0f - camDistance), ref camLocalPosVel, 0.02f);
			setCameraLocalRotSmooth(0.4f);
		}
	}

	private void ShowAndHideSlide()
	{
		if (aiPlaying) powerMetersParentRectTrans.anchoredPosition = new Vector2(130.0f, -70.0f);
		if (!aiPlaying) powerMetersParentRectTrans.anchoredPosition = new Vector2(-130.0f, -70.0f);
	}

	void GetAllBallsVelocity()
	{
	...
	private void setCameraLocalRotSmooth(float timeVal)
	{
		...
		localEulerAngles.x = Mathf.SmoothDampAngle(localEulerAngles2.x, 0f, ref camLocalRotVel.x, timeVal);
		Vector3 localEulerAngles3 = cameraObjTransform.localEulerAngles;
		localEulerAngles.y = Mathf.SmoothDampAngle(localEulerAngles3.y, 0f, ref camLocalRotVel.y, timeVal);
		Vector3 localEulerAngles4 = cameraObjTransform.localEulerAngles;
		localEulerAngles.z = Mathf.SmoothDampAngle(localEulerAngles4.z, 0f, ref camLocalRotVel.z, timeVal);
	...
	private void resetCueBallToStart()
	{
		if (gameMode == GAME_MODE.SNOOKER)
		{
			thisTransform.position = CUEBALL_START_SNOOKER_POS + new Vector3(_camera.ScreenPointToRay(Input.mousePosition).direction.x, 0f, 0f);
		}
		else
		{
	...
	//Slide de Power na tacada !
	private void switchControls()
	{
		setInitialStrikePower();
		if (aiPlaying)
		{
			powerMetersParentRectTrans.position = new Vector3(-130.0f, -70.0f, 0.0f);
		}
		if (!aiPlaying)
		{
			powerMetersParentRectTrans.position = new Vector3(-130.0f, -70.0f, 0.0f);
	...
	private void aiReboundTurningInvoke()
	{
		cueRotValueX += 0.8f;
		cueParentObjTransform.eulerAngles = new Vector3(0f, cueRotValueX, 0f);
		if (Physics.SphereCast(thisTransform.position, 0.49f, cueParentObjTransform.forward, out RaycastHit hitInfo, 100f, ballLineLayerMask) && hitInfo.collider.CompareTag("colSideTag") && Physics.SphereCast(guideColRingPosVec, 0.49f, guideReflectDirVec, out RaycastHit hitInfo2, 100f, ballsLayerMask))
		{
	...
	private void hitTheBall(float shotPower)
	{
		...
		if (!aiPlaying)
		{
			...
			aimColRingPosOnHit = guideColRingPosVec;
			cueBallReboundVector = guideDirCueBallTrans.forward;
			thisRigidbody.velocity = _camController.taco.transform.forward * 100f * shotPower;
			if (Physics.SphereCast(thisTransform.position, 0.49f, _camController.taco.transform.forward, out lineHit,
				    100f, ballsLayerMask))
			{
				checkFixedUpdateBallTouch = true;
				velocityOnHit = thisRigidbody.velocity.magnitude;
				firstTargetBallToHit = lineHit.collider.gameObject;
				angleOnHit = Vector3.Angle(_camController.taco.transform.forward, -lineHit.normal);
				cueBallToAimColRingDistanceOnHit = Vector3.Distance(cueBallPosOnHit, aimColRingPosOnHit);
				cueBallHitVector = _camController.taco.transform.forward;
			}
			else
			{
				checkFixedUpdateBallTouch = false;
	...
	private void updateGuide()
	{
		...
		Vector3 eulerAngles = _camController.taco.transform.rotation.eulerAngles;
		transform2.rotation = Quaternion.Euler(0f, eulerAngles.y, 0f);
		if (!Physics.SphereCast(thisTransform.position, 0.49f, _camController.taco.transform.forward, out lineHit, 100f, ballLineLayerMask))
		{
			return;
		}
		guideReflectDirVec = Vector3.Reflect(_camController.taco.transform.forward, lineHit.normal).normalized;
		...
		if (lineHit.collider.CompareTag("ballTag"))
		{
			...
			guideTempAngle = Vector3.Angle(_camController.taco.transform.forward, -lineHit.normal);
			Vector3 eulerAngles2 = guideDirCueBallTrans.eulerAngles;
			Vector3 vector = Vector3.Cross(-lineHit.normal, thisTransform.position - (lineHit.point - lineHit.normal * 0.5f));
			if (vector.y > 0f)
			{
			...
			{
				Vector3 vector2 = Vector3.Cross(-lineHit.normal, thisTransform.position - (lineHit.point - lineHit.normal * 0.5f));
				if (vector2.y < 0f)
				{
					if (guideTempAngle <= 5f)
					{
